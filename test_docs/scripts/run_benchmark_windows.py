...
def main() -> None:
    ...
    # Windows: use CREATE_NEW_PROCESS_GROUP instead of start_new_session
    ...
    try:
        ...
        run_command([sys.executable, "test_docs/scripts/benchmark_capture.py"], env=env, cwd=ROOT)
        ...
        if not args.keep_server and server.poll() is None:
            # Windows: use CTRL_BREAK_EVENT instead of SIGTERM on process group
